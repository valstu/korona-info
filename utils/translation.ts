...
const phrases: { [key: string]: { [key: string]: string } } = {
  ...
  ru: {
    ...
    'less people will be tested':
      'Надежность статистики может меняться в некоторых районах, к примеру в районе Хельсинки было принято решение тестировать только некоторые группы,в частности работников сферы мед.обслуживание, на COVID-19.',
